...
**Resume rules:**
- When the user asks for a resume, ALWAYS render a PDF via `resume render`. Never dump raw JSON/text as the resume output.
- The bot auto-detects PDF paths in your output and sends them as Telegram documents.
- Text commentary (gaps, suggestions, what changed) goes in your response text alongside the PDF.
- Default output goes to `data/output/{name}_{id[:8]}.pdf`. The path is stored in the DB (`pdf_path` column).
- Resume content is stored as JSON in the DB matching the Typst template schema.
- For tailored resumes: create a new resume entry with a descriptive name (e.g., `meta-swe`, `databricks-ml`), render it, then the user gets a unique PDF.
...
- **"Show me my resume"**: `resume render base` → PDF sent via bot
- **"Tailor my resume for this job"**: Read job desc + base resume → create tailored JSON → `resume add --name "company-role"` → `resume render company-role` → PDF sent. Walk the user through each step conversationally, offering to adjust before rendering.
...
source_data/        # raw resume JSONs, LinkedIn dumps, product docs
```
